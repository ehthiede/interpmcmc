...
class InterpForceField(object):
    ...
    def F(self, x, y):
        f_x = self.rbs(x, y, grid=False, dx=1) 
        f_y = self.rbs(x, y, grid=False, dy=1) 
        return f_x, f_y

def build_padded_ax(dmn, npnts, pad_width):
    dx = (dmn[1] - dmn[0])  / npnts_x
...
def pad_pmf(pmf, domain, pad_width = 3, clip_val=20):
    ...
    padded_pmf = np.pad(pmf, pd, mode='wrap')
 
    return padded_pmf, xax_padded, yax_padded
    #stack the pmf to account for periodic boundary conditions.
    
def build_rbs(pmf, domain, pad_width = 3, clip_val=20.):
    pmf, xax, yax = pad_pmf(pmf, domain, pad_width, clip_val=20.)
    
    rbs = RectBivariateSpline(xax, yax, pmf)
    return rbs
   
def build_ala_forcefield():
    pmf = np.load('mean_pmf.npy')
    domain = ((-np.pi, np.pi), (-np.pi, np.pi))
    rbs = build_rbs(pmf, domain)
    return InterpForceField(rbs)
